...
void printPolynomial(double x, double c[], int n) 
{
  ...
  for(int i=0; i<n; i=i+1) {
	  ...
	  if (c[i]>0) cout << " + ";  //print "+" if necessary 
...
/*
Derives the polynomial and returns the new degree after derivation.

*/
void derPolynomial(double c[], int n) {
  ...
  for (int i=0; i<n; i++) {
    ...
    c[i] *= (n-i); 
    // Example: 4*x^3 + 0 + 0 + 0: n=4, c=4; n=3; i=0, c=3*4;
...
int main(int argc, char* argv[])
{
  ...
  //the loop continually lowers the degree for the derivation below
  for (i=numCoeffs; i >= 1; i--) {

  //Now we can print the polynom by calling the function printPolynom
  printPolynomial(x, coeffs,i);

  //...and its result
  cout << " = " << evalPolynomial(x,coeffs,i) << endl;
  //note, that we call a function anywhere in an expression.
  //derive the polynomial, changing its coeffs

  derPolynomial(coeffs,i);
  }
