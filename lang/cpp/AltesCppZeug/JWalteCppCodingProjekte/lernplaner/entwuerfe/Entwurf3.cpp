...
//nötig für die Funktion balance_people, siehe unten
...
//____________________________________________________________
//Maß-Funktionen START


void make_random(motherrowdef& therandommother, groupdef thegroup, unsigned int the_m, unsigned int the_n) {
  //macht auch eine motherrow, aber die verteilung ist zufällig

  cout << "    ********************" << endl;
  cout << "    Entered make_random" << endl;
  cout << "    ********************" << endl;

  unsigned int randnr;
  //der zufallsbasierte index

  vector<unsigned int> freepositions;
  //speichert die anfänglichen array-Indizes der freien Sitze in der randommother. Sobald welche belegt werden, fallen sie hier ebenfalls raus

  cout << "    Before distributing the people, the free positions in randommother are this:" << endl << "    ";

  for (unsigned int j =0; j < the_n; j++) {
    freepositions.push_back(j);
    cout << freepositions[j] << ' ';
  }cout << endl << endl;

  vector<unsigned int> peoplefromgroup;
  //speichert, wieviele Leute der jeweiligen Gruppe bereits untergebracht wurden

  for (unsigned int i=0; i < the_m; i++) {
    //laufe durch alle Gruppen

    peoplefromgroup.push_back(0);
    //mache Platz für die Leute aus der nächstbetrachteten Gruppe

    while (peoplefromgroup[i] < thegroup[i].second) {
      //Solange noch Leute aus der Gruppe nicht untergebracht wurden:

      randnr = static_cast<unsigned int>(rand() % freepositions.size());
      //generiere eine zufällige Zahl für den Index der freien Sitze
      //Der Wert von freeposition[rand] ist dann der ursprüngliche Index in randommother
      //rand() % x gibt eine zufallszahl e [0,x-1] zurück.

      therandommother[freepositions[randnr]] = thegroup[i].first;
      //füge den namen der gruppe auf dem sitz ein

      peoplefromgroup[i]++;

      freepositions.erase(freepositions.begin()+randnr);
      //lösche diesen sitz aus denen dem speicher der freien sitze (random access!)

    }

  }

  cout << "    The randommother now looks like this: " << endl << "    ";
  for (unsigned int j=0; j < the_n; j++)
    cout << therandommother[j] << ' ';
  cout << endl << endl;

}


double make_measure(motherrowdef themotherrow, groupdef thegroup, unsigned int the_m, unsigned int the_n){
  //berechne ein Gleichverteilungsmass für eine gegebene Verteilung aller Gruppen motherrow

  cout << "    ********************" << endl;
  cout << "    Entered make_measure" << endl;
  cout << "    ********************" << endl;


  unsigned int indexup, indexdown, i,j,u;
  int indexdown_signed;

  vector<double> measure(the_n);
  for (j=0; j < the_n; j++) measure[j] = 0;

  vector<double> sigma(the_m);

  double measure_average = 0.0;
  double sigma_average = 0.0;

  for (i=0; i < the_m; i++) {
    //tue folgendes für jede gruppe
    for (j = 0; j < the_n; j++) {
      //von jeder sitzposition in motherrow aus:
      for (u = 0; u <= the_n/2; u++) {
	//siehe bei allen nachbarsitzen nach, ob eine person aus der gruppe i dort sitzt.

	indexup = (j+u) % the_n; 
	//der index für das nachschauen in aufwärts-richtung (zyklisch)

	indexdown_signed = (static_cast<int>(j)-static_cast<int>(u)) % static_cast<int>(the_n);
	if (indexdown_signed < 0) indexdown_signed += the_n;
	indexdown = static_cast<unsigned int>(indexdown_signed);
	//der index für das nachschauen in abwärts-richtung (zyklisch)
	//da C++ für (negativezahl % n) ein negatives ergebnis liefert, macht diese klausel daraus das gewohnte modulo-Verhalten, bspw -1 % 5 == 4 und nicht -1.

	if (themotherrow[indexup].compare(thegroup[i].first) == 0) {
	  //wenn auf diesem vom Sitz j etwas entfernten Sitz eine Person der Gruppe i sitzt (str1.compare(str2) == 0 bedeutet: beide strings identisch)
	  measure[j] += ((double)ceil( static_cast<double>(the_n) / 2.0  ) + (double)( (the_n+1) % 2 ) - (double)u)/(double)thegroup[i].second;
	  //wichte ihn proportional zum Abstand zum Sitz j und füge diese Wichtung zum Mass für den j-ten Sitz dazu. (Bsp: j erhält Wichtung 7, j+1 und j-1 (siehe Klausel unten) Wichtung 6 usw. Wenn auf einem dieser Sitze eine Person aus Gruppe i sitzt, wird diese Wichtung zur j-ten hinzugefügt.
	  measure_average += measure[j];
	  //das ist für die Normalisierung auf 1, s.u.

	}


	if (themotherrow[indexdown].compare(thegroup[i].first) == 0) {
	  //mach dasselbe in die abwärts steigende richtung von sitz j aus gesehen
	  measure[j] += (double)ceil( static_cast<double>(the_n) / 2.0  ) + (double)( (the_n+1) % 2 ) - (double)u;
	  measure_average += measure[j];
	}
    ...
    //nun sind die Wichtungen aller Sitze für die Leute einer Gruppe i berechnet.

    cout << "    The unnormalized measure for group " << thegroup[i].first << " now looks like this:" << endl << "    ";
    for (j=0; j < the_n; j++)
      cout << measure[j] << ' ';
    cout << endl << endl;
    
    measure_average /= (double)the_n;
    //mit diesem faktor werden die measure[j] auf 1 normalisiert

    for (j=0; j < the_n; j++) sigma[i] += pow(( measure[j]/measure_average - 1.0  ),2);
    //der innere Teil der normalen Standardabweichungsformel (-1 und nicht - average, weil schon normalisiert)
    
    sigma[i] = sqrt( 1/(double)the_n * sigma[i]  );
    //der äußere Teil

    sigma_average += sigma[i];
    //mit diesem faktor werd die Gesamtstandardabw. normalisiert

  }//i-loop END

  cout << "The unnormalized sigma of the normalized measures for the different groups now look like this: " << endl << "    ";
  for (i=0; i < the_m; i++)
    cout << sigma[i] << ' ';
  cout << endl << endl;

  //nun sind die Standardabweichungen für alle i Gruppen in sigma[i] gespeichert.
  //ich bin aber nur am Mittelwert interessiert. Also

  return sigma_average / (double)the_m;

}

void grade(motherrowdef& themotherrow, groupdef thegroup, unsigned int the_m, unsigned int the_n, unsigned int the_randomsamples) {

  double sigma;

  cout << endl << "__________________________________________________" << endl;
  cout << "            Entering grade" << endl;
  cout  << "__________________________________________________" << endl;


  sigma = make_measure(themotherrow, thegroup, the_m, the_n);

  cout << "Back in grade(). Now make " << the_randomsamples << " random configurations for comparison." << endl << endl;
  
  double randommeasureaverage = 0.0;
  motherrowdef randommother;

  for (unsigned int a = 0; a < the_randomsamples; a++) {

    cout << "  Make Random configuration No. " << a << endl;

    for (unsigned int j = 0; j < the_n; j++)
      randommother.push_back("0");

  make_random(randommother, thegroup, the_m, the_n);

  cout << "  Back in grade(). Now compute the random configurations' respective sigmas." << endl;

  randommeasureaverage += make_measure(randommother, thegroup, the_m, the_n);

  if (a != the_randomsamples-1)
  cout << "  Back in grade(), still in loop: make next random configuration." << endl << endl;

  randommother.clear();

  }

  randommeasureaverage /= the_randomsamples;

  cout << endl << "Back in grade. Now compare: " << endl << endl;
  cout << "The optimized versions's sigma is: " << sigma << endl;
  cout << "The average sigma of the " << the_randomsamples << " random configurations is: "<< randommeasureaverage << endl;
  cout << "The ratio is sigmaopt / sigmaav (should be smaller 1): " << endl;
  cout << sigma / randommeasureaverage << endl << endl;

}



//Maß-Funktionen FINISH
//FINISH
//____________________________________________________________
...
int main () {
  ...
  //ANALYSE05: Die Gruppen mit großem people[i] haben schlechte Maße. Das liegt wohl zum einen daran, dass der schnitt people[i]/n (n motherrowlength) sich einfach negativ auswirkt, und zum anderen eventuell daran, dass ich die großen Brocken zuletzt verteile. Versuche mal, die Sortierung umzukehren, und schauen, wie sich das auswirkt aufs Maß.
  vector<unsigned int> reversetmp;
  for (unsigned int i=0; i<m; i++)
    reversetmp.push_back(group[i].second);
  reverse(reversetmp.begin(),reversetmp.end());
  for (unsigned int i=0; i<m; i++)
    group[i].second = reversetmp[i];
  ...
  cout << "Equidistribution measure:" << endl;

  unsigned int randomsamples = 10;

  grade(motherrow, group, m, n, randomsamples);
  //TODO
