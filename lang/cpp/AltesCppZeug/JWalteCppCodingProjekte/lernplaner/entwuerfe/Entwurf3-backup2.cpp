...
double make_measure(motherrowdef themotherrow, groupdef thegroup, unsigned int the_m, unsigned int the_n){
  ...
  unsigned int index;
  vector<unsigned int> weights(the_n);
  vector<double> average(the_m);
  double sigma = 0.0;
  ...
  for (unsigned int i = 0; i < the_m; i++) {
    for (unsigned int j = 0; j < the_n; j++) {
      for (unsigned int u = 1; u <= static_cast<unsigned int> (ceil((double)the_n/(double)2)+(double)((the_n+1) % 2)); u++) {

	index = j + u % the_n;

	if (themotherrow[index] == thegroup[i].first);
	weights[j] += static_cast<unsigned int>((ceil((double)the_n/(double)2)+((the_n+1 % 2)) - u));

      }

      for (unsigned int u = static_cast<unsigned int>(ceil((double)the_n/2))+((the_n+1)%2); u > 0; u--) {
	...
	weights[j] += u;
      ...
      for (unsigned int j=0; j < the_n; j++)
	average[i] += static_cast<double>(weights[j]);
      average[i] /= static_cast<double>(the_n);
      sigma += average[i];
...
void grade(motherrowdef& themotherrow, groupdef thegroup, unsigned int the_m, unsigned int the_n) {


  cout << endl << "__________________________________________________" << endl;
  cout  << "__________________________________________________" << endl;
  cout << "Gleicherteilung:              sigma = " << make_measure(themotherrow, thegroup, the_m, the_n) << endl;
  cout << "Vergleich Zufallsverteilung:  sigma = " << make_measure(themotherrow, thegroup, the_m, the_n) << endl;

  unsigned int randomsamples = 10;
  double randommeasureaverage = 0.0;
  motherrowdef randommother;

  for (unsigned int a = 0; a < randomsamples; a++) {
  ...
  randommeasureaverage /= randomsamples;

  cout  << "__________________________________________________" << endl;
  cout << "Vergleich Zufallsverteilung:  sigma = " << randommeasureaverage << endl;
  cout << "(Anzahl Samples: " << randomsamples << " )" << endl;
  cout  << "__________________________________________________" << endl;
}
